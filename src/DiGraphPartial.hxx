...
class DiGraphPartial {
  template <class T>
  using Bitset = BitsetPartial<T>;
...
void write(ostream& a, const DiGraphPartial<V, E>& x, bool all=false) {
...
template <class V, class E>
ostream& operator<<(ostream& a, const DiGraphPartial<V, E>& x) {
  write(a, x);
  return a;
}
